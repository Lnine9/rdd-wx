<template>
	<view>
		...
		<view class="commodity-inner-container">
			<view class="commodity-img-container"><image :src="imageUrl" mode="" class="commodity-img"></image></view>
			<view class="commodity-content-container">
				<!-- 商品名 -->
				<text class="commodity-title">{{ order.commodityTitle }}</text>
			</view>
		</view>
		<!-- 快递信息 -->
...
export default {
	...
	methods: {
		...
		judgeTraces(data) {
			...
			console.log(data.deliveryNum != null && data.deliveryCompany != null && data.deliveryNum != '' && data.deliveryCompany != '');
			if ((data.deliveryNum == null && data.deliveryCompany == null) || (data.deliveryNum == '' && data.deliveryCompany == '')) {
				// this.getTraces();
		...
		getTraces: function() {
			let _this = this;
			DeliverAPI.getDeliverInfo(this.requestParams)
				.then(res => {
					if (res.data.data) {
